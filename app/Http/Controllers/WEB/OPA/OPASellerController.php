...
use Mail;
use Str;
class OPASellerController extends Controller
{
    public function __construct()
    {
        $this->middleware('auth:opa');
    }

    public function index(){
        $user = Auth::guard('opa')->user();
        $opa = OPA::where('email', '=', $user->email)->first();
        $referral_code = $opa->referral_code;
        $sellers = Vendor::with('user','socialLinks','products')->orderBy('id','desc')->where('referral', $referral_code)->where('status',1)->get();
        $defaultProfile = BannerImage::whereId('15')->first();
        $products = Product::all();
        $setting = Setting::first();

        return view('opa.seller', compact('sellers','defaultProfile','products','setting'));
    ...
    public function create(){
        ...
        return view('opa.create_seller', compact('referral_code'));
    }
    ...
    public function pendingSellerList(){
        ...
        $opa = OPA::where('email', '=', $user->email)->first();
        $referral_code = $opa->referral_code;
        $sellers = Vendor::with('user','socialLinks','products')->orderBy('id','desc')->where('referral', $referral_code)->where('status',0)->get();
        $defaultProfile = BannerImage::whereId('15')->first();
        $products = Product::all();
        $setting = Setting::first();
