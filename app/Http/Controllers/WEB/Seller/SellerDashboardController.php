...
class SellerDashboardController extends Controller
{
    ...
    public function index(){
        ...
        $vendor = Vendor::where('id', $user->seller->id)->first();
        if($vendor->details_filled == 0){
            $notification=trans('admin_validation.Enter Details');
            $notification=array('messege'=>$notification,'alert-type'=>'info');
            return redirect()->route('seller.initial-details')->with($notification);
        }
