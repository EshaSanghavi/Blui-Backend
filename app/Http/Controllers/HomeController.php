...

{
    ...
    public function websiteSetup(){
        ...
        $productCategories = Category::with('activeSubCategories.activeChildCategories')->where(['status' => 1])->select('id','name','slug','icon', 'priority')->orderBy('priority', 'asc')->get();

        $megaMenuCategories = MegaMenuCategory::with('category','subCategories.subCategory')->orderBy('priority','asc')->where('status',1)->get();
    ...
    public function subCategoriesByCategory($id){
        ...
        $subCategories = SubCategory::where(['category_id' => $id, 'status' => 1])->orderBy('priority', 'asc')->get();
    ...
    public function childCategoriesBySubCategory($id){
        ...
        $childCategories = ChildCategory::where(['sub_category_id' => $id, 'status' => 1])->orderBy('priority', 'asc')->get();
    ...
    public function productBySubCategory($id){
        ...
        $products = Product::with('activeVariants.activeVariantItems')->where(['sub_category_id' => $sub_category->id, 'status' => 1,'approve_by_admin' => 1])->select('id','name', 'short_name', 'slug', 'thumb_image','qty','sold_qty', 'price', 'offer_price','is_undefine','is_featured','new_product', 'is_top', 'is_best','category_id','sub_category_id','child_category_id','brand_id')->orderBy('id','desc')->get();
    ...
    public function productByChildCategory($id){
        ...
        $child_category = ChildCategory::where('slug', '=', $id)->first();

        $products = Product::with('activeVariants.activeVariantItems')->where(['child_category_id' => $child_category->id, 'status' => 1,'approve_by_admin' => 1])->select('id','name', 'short_name', 'slug', 'thumb_image','qty','sold_qty', 'price', 'offer_price','is_undefine','is_featured','new_product', 'is_top', 'is_best','category_id','sub_category_id','child_category_id','brand_id')->orderBy('id','desc')->get();
...

    {
        ...
        $homepage_categories = Category::where(['status' => 1])->select('id','name','slug','icon','image', 'priority')->orderBy('priority', 'asc')->get()->take(15);
    ...
    public function sellerDetail(Request $request, $shop_name){
        ...
        $categories = Category::with('activeSubCategories.activeChildCategories')->where(['status' => 1])->select('id','name','slug','icon', 'priority')->orderBy('priority', 'asc')->get();
    ...
    public function productDetail($slug){
        ...
        $attributes = ProductAttribute::where('product_id', $product->id)
            ...
            ->select('product_attributes.*', 'attributes.name as name', 'attributes.type as type', 'attributes.priority as priority')
